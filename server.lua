...
lib.callback.register('ox_inventory:renameItem', function(source, slot, name)
	local inventory = Inventory(source)
	if not inventory then return end
	...
	if not Config.Onyx.Items.GlobalRename and (not itemDef or not itemDef.rename) then return end

	if type(name) ~= 'string' or #name < 1 or #name > 30 then return end

	local metadata = item.metadata
	metadata.label = name

	Inventory.SetMetadata(inventory.id, slot, metadata)
	return true
end)
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			if consume and consume ~= 0 and not data.component then
				...
				if not durability then
					local removeCount = consume < 1 and 1 or consume
					Inventory.RemoveItem(inventory.id, data.name, removeCount, nil, data.slot)
				
					-- Handle item replacement (e.g., water_bottle -> empty_bottle)
					if item.replace then
						local replacementItem = Items(item.replace)
						if replacementItem then
							-- Try to add the replacement item to the inventory
							local success = Inventory.AddItem(inventory.id, item.replace, removeCount)
							if not success then
								-- If inventory is full, notify the player
								TriggerClientEvent('ox_lib:notify', source, { 
									type = 'warning', 
									description = locale('cannot_carry_item', replacementItem.label)
								})
							end
						end
					end
