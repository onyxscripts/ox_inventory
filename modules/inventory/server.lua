...
local function generateItems(inv, invType, items)
	if items == nil then
		print('[OxInv] Generating items for', invType)
		if invType == 'dumpster' then
			if Config.Onyx.Dumpsters.Enabled ~= false then
				items = randomLoot(server.dumpsterloot)
			else
				items = {} -- Empty dumpsters if disabled via Onyx config
			end
			print('[OxInv] Generated items count:', #items)
...
function Inventory.Load(id, invType, owner)
	...
	elseif invType == 'dumpster' then
		print('[OxInv] Loading dumpster. randomloot:', server.randomloot)
		if server.randomloot then
...
---@param suppressSync? boolean
---@return boolean? success, string|SlotWithItem|nil response
function Inventory.AddItem(inv, item, count, metadata, slot, cb, suppressSync)
	if type(item) ~= 'table' then item = Items(item) end
...
exports('CreateDropFromPlayer', function(playerId)
	...
	local model = firstItem and Items(firstItem.name).dropProp
...
local function dropItem(source, playerInventory, fromData, data)
	...
	-- Set playerInventory.open to the new drop so that subsequent swapItems
	-- calls resolve Inventory(playerInventory.open) to this drop instead of
	-- the player's own inventory (which causes a corrupted self-swap).
	playerInventory:openInventory(inventory)

	inventory.coords = data.coords
	local itemModel = Items(toData.name) and Items(toData.name).dropProp
    local model = (Config.Onyx and Config.Onyx.DropProps.Enabled and itemModel) or (Config.Onyx and Config.Onyx.DropProps.DefaultModel) or Inventory.Drops[math.random(1, #Inventory.Drops)].model
    local itemLabel = toData.metadata and toData.metadata.label or Items(toData.name) and Items(toData.name).label or toData.name
	Inventory.Drops[dropId] = {coords = inventory.coords, instance = data.instance, model = model, label = itemLabel, slot = data.toSlot, name = toData.name, count = data.count}
	playerInventory.changed = true

	TriggerClientEvent('ox_inventory:createDrop', -1, dropId, Inventory.Drops[dropId], nil, slot)
...
lib.callback.register('ox_inventory:craftFromDrag', function(source, data)
	if not Config.Onyx or not Config.Onyx.DragToCraft.Enabled then 
		return false 
	end
	...
	local fromSlotData = playerInventory.items[data.fromSlot]
	local toSlotData = playerInventory.items[data.toSlot]

	if not fromSlotData or not toSlotData then 
		return false 
	end
	if fromSlotData.name ~= data.fromItem or toSlotData.name ~= data.toItem then 
		return false 
	end

	local recipes = Config.Onyx.DragToCraft.Recipes
	if not recipes then 
		return false 
	end

	local fromItemMatch = data.fromItem:lower()
	local toItemMatch = data.toItem:lower()
	...
	for i, recipe in ipairs(recipes) do
		...
		if materials then
			...
			for k, v in pairs(materials) do 
				matCount = matCount + 1 
				normMaterials[k:lower()] = v
			end

			if matCount == 2 and fromItemMatch ~= toItemMatch and normMaterials[fromItemMatch] and normMaterials[toItemMatch] then
				validRecipe = { recipe = recipe, norm = normMaterials }
				break
			elseif matCount == 1 and fromItemMatch == toItemMatch and normMaterials[fromItemMatch] then
				validRecipe = { recipe = recipe, norm = normMaterials }
				break
			end
		end
	...
	local actualRecipe = validRecipe.recipe
	local normMaterials = validRecipe.norm
	local reqSource, reqTarget

	if fromItemMatch ~= toItemMatch then
		reqSource = normMaterials[fromItemMatch]
		reqTarget = normMaterials[toItemMatch]

		if fromSlotData.count < reqSource or toSlotData.count < reqTarget then
			return false, 'not_enough_materials'
		end

		print('REMOVING ITEMS (2)')
		Inventory.RemoveItem(playerInventory, data.fromItem, reqSource, nil, data.fromSlot)
		Inventory.RemoveItem(playerInventory, data.toItem, reqTarget, nil, data.toSlot)
	else
		local totalReq = normMaterials[fromItemMatch]
		if fromSlotData.count + toSlotData.count < totalReq then
			print('FAILED: Not enough materials (1). Have combined', fromSlotData.count + toSlotData.count, 'Need', totalReq)
			return false, 'not_enough_materials'
		end

		print('REMOVING ITEMS (1)')
		local targetRem = math.min(toSlotData.count, totalReq)
		local sourceRem = totalReq - targetRem

		if targetRem > 0 then Inventory.RemoveItem(playerInventory, data.toItem, targetRem, nil, data.toSlot) end
		if sourceRem > 0 then Inventory.RemoveItem(playerInventory, data.fromItem, sourceRem, nil, data.fromSlot) end
	end

	local chance = actualRecipe.Chance or 100
	local rolled = math.random(1, 100)
	if rolled <= chance then
		Inventory.AddItem(playerInventory, actualRecipe.Result, actualRecipe.ResultAmount or 1)
	end
	...
	return true, {
		...
		items = {
			{
				item = playerInventory.items[data.fromSlot] or { slot = data.fromSlot },
				inventory = playerInventory.id
			},
			{
				item = playerInventory.items[data.toSlot] or { slot = data.toSlot },
				inventory = playerInventory.id
			}
		}
	}
...
RegisterNetEvent('ox_inventory:pickupDrop', function(dropId, slotIndex)
    local source = source
    local playerInventory = Inventory(source)
    local dropInventory = Inventory(dropId)

    if not playerInventory or not dropInventory or dropInventory.type ~= 'drop' then return end

    local pedCoords = GetEntityCoords(GetPlayerPed(source))
    if #(pedCoords - dropInventory.coords) > 5.0 then return end

    local successTotal = 0
    for slot, item in pairs(dropInventory.items) do
        if not slotIndex or slot == slotIndex then
            if item then
                local toSlot = Inventory.GetSlotForItem(playerInventory, item.name, item.metadata)
                if Inventory.AddItem(playerInventory, item.name, item.count, item.metadata, toSlot) then
                    Inventory.RemoveItem(dropInventory, item.name, item.count, item.metadata, slot)
    ...
    if successTotal > 0 then
        lib.logger(playerInventory.owner, 'pickupDrop', ('Picked up %d items from drop %s'):format(successTotal, dropId))

        if not next(dropInventory.items) then
            Inventory.Remove(dropInventory)
        end
    end
end)
