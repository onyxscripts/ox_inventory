...
function Utils.ItemNotify(data)
    ...
    local key = ('%s_%s'):format(data[1].name, data[2])

    if not notificationQueue[key] then
        local isEquipOrHolster = data[2] == 'ui_equipped' or data[2] == 'ui_holstered'
        local initCount = 0
        if isEquipOrHolster then initCount = nil end
    ...
    if notificationQueue[key].count then
        notificationQueue[key].count = notificationQueue[key].count + (data[3] or 1)
    end

    if not notificationTimer then
        notificationTimer = SetTimeout(DEBOUNCE_TIMER, function()
            for _, v in pairs(notificationQueue) do
                local notificationData = v.data
                notificationData[3] = v.count
                SendNUIMessage({ action = 'itemNotify', data = notificationData })
            end
            notificationQueue = {}
