...
function client.openInventory(inv, data)
    ...
    if Config.Onyx and Config.Onyx.UI and Config.Onyx.UI.Blur ~= false and client.screenblur then Utils.blurIn() end
...
local function parseDuration(durationStr)
    ...
    if unit == 's' then return number * 1000 end
    if unit == 'm' then return number * 60000 end
    return number * 1000 -- Default to seconds
end
...
RegisterNUICallback('renameItem', function(data, cb)
	...
	local input = lib.inputDialog(locale('rename_item'), {
		{ type = 'input', label = locale('new_name'), required = true, min = 1, max = 30 }
	})
	SetNuiFocusKeepInput(true)

	if not input then return end

	lib.callback.await('ox_inventory:renameItem', 200, data.slot, input[1])
end)
...
local function useSlot(slot)
    ...
    local cooldown = item.metadata?.cooldown or Items[item.name]?.cooldown
    if cooldown then
        local now = GetGameTimer()
        local expiration = activeCooldowns[item.name]
        
        if expiration and now < expiration then
            local remaining = math.ceil((expiration - now) / 1000)
            lib.notify({ type = 'error', description = ('Item is on cooldown (%ss)'):format(remaining) })
            return
        end

        local duration = parseDuration(cooldown)
        if duration > 0 then
            activeCooldowns[item.name] = now + duration
            SendNUIMessage({
                action = 'itemCooldown',
                data = {
                    name = item.name,
	...
	if not point.instance or point.instance == currentInstance and not point.entity then
        ...
        if shared.target then
            ...
            exports.ox_target:addSphereZone({
                ...
                radius = 0.35,
                debug = false,
                name = 'ox_inventory:drop_' .. point.invId,
                options = {
                    {
                        name = 'ox_inventory:pickupdrop_' .. point.invId,
                        label = 'Pickup ' .. (point.count and (point.count .. 'x ') or '') .. (point.label or 'Drop'),
                        icon = 'fas fa-hand-holding',
                        distance = 2.0,
                        onSelect = function()
                            TriggerServerEvent('ox_inventory:pickupDrop', point.invId, point.slot)
                        end
                    }
                }
            })
...
local function onExitDrop(point)
	...
	if entity then
        if shared.target then
            exports.ox_target:removeZone('ox_inventory:drop_' .. point.invId)
        end
...
RegisterNetEvent('ox_inventory:createDrop', function(dropId, data, owner, slot)
	...
	if owner == cache.serverId then
		...
		if invOpen and #(GetEntityCoords(playerPed) - data.coords) <= 1 then
			if not cache.vehicle then
				-- Block swapItems during the openInventory server callback
				-- to prevent a race where playerInventory.open is stale
				plyState:set('invBusy', true, true)

				-- Call the server to set playerInventory.open to the new drop,
				-- but ONLY update the right inventory in the NUI.
				-- Using client.openInventory here would resend leftInventory with
				-- potentially stale PlayerData.inventory (the swapItems callback
				-- that updates PlayerData hasn't returned yet in the other coroutine),
				-- which overwrites the already-correct NUI left inventory state.
				local left, right = lib.callback.await('ox_inventory:openInventory', false, 'drop', { id = dropId })

				if right then
					currentInventory = right
					SendNUIMessage({
						action = 'setupInventory',
						data = { rightInventory = right }
					})
				end
...
-- Disable player controls when search bar is focused
RegisterNUICallback('setSearchFocus', function(focused, cb)
...
RegisterNUICallback('giveItem', function(data, cb)
    ...
    if time - lastGive < 500 then return cb(0) end
    lastGive = time
	...
	if client.giveplayerlist then
		local nearbyPlayers = lib.getNearbyPlayers(GetEntityCoords(playerPed), 1.5)
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	if data.toType == 'newdrop' then
        ...
        local offsetX = (math.random() * 1.0) - 0.5
        local offsetY = (math.random() * 1.0) - 0.5
        coords = vec3(coords.x + offsetX, coords.y + offsetY, coords.z)
...
RegisterNUICallback('craftFromDrag', function(data, cb)
	if invBusy or not invOpen then return cb(false) end
	
	local craftTime = data.duration or 0
	if craftTime == 0 and Config.Onyx and Config.Onyx.DragToCraft and Config.Onyx.DragToCraft.CraftingTime then
		craftTime = Config.Onyx.DragToCraft.CraftingTime
	end
	local craftLabel = data.label or locale('ui_crafting') or 'Crafting...'
	...
	local success, response = lib.callback.await('ox_inventory:craftFromDrag', 200, data)
